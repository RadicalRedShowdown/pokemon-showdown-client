...
const Dex = new class implements ModdedDex {
	...
	fxPrefix = (() => {
		if (window.document?.location?.protocol === 'file:') {
			if (window.Replays) return `http://${window.Config ? Config.routes.client : 'play.pokemonshowdown.com'}/fx/`;
			return `fx/`;
		}
	...
	resolveAvatar(avatar: string): string {
		...
		if (avatar.includes('.')) {
			...
			return 'https://sim.radicalred.net:8443/avatars/' + encodeURIComponent(avatar).replace(/\%3F/g, '?');
		}
