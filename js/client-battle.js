(function ($) {
	...
	var BattleOptionsPopup = this.BattleOptionsPopup = Popup.extend({
		initialize: function (data) {
			...
			buf += '<p><label class="optlabel"><input type="checkbox" name="statstages"' + (Dex.prefs('statstages') ? ' checked' : '') + ' /> Show stat stages</label></p>';
		...
		events: {
			...
			'change input[name=statstages]': 'toggleStatStages'
		},
		...
		toggleStatStages: function (e) {
			Storage.prefs('statstages', !!e.currentTarget.checked);
		}
	});
